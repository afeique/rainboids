...
- **20 enemy types** with distinct movement, attack patterns, and visual designs (6.71.0–6.83.0 — added the elemental **Cinder** fire-swarm, **Glacier** ice-tank, **Frost Lance** ice-sniper, **Ashen Detonator** fire-bomber (death-flare), **Tesla Wraith** volt-skirmisher, **Plaguebearer** toxic mine-layer that leaves corrosive acid trails, the anti-meta **Warden** that adapts its resistance to the element you keep using — forcing you to switch — the **Hydra** that splits into smaller copies on death, the **Spore Carrier** that births drone swarms, and the **Lumen Drone** that shields nearby allies (kill it first); more rolling out). Enemy visuals stay at base size at every wave (5.101.0) — only HP / damage / speed scale.
- **Element & resistance system (6.57.0–6.67.0)** — 7 damage elements (Kinetic, Pyro, Cryo, Volt, Toxic, Void, Radiant), each with a signature status. Enemies carry per-element **resistances and weaknesses** (e.g. Guardian shorts out to Volt, Tangerine is fireproof, Wasps freeze-shatter), so bringing the right element matters. A status engine (Burn, Corrode armor-shred, Chill/Freeze, Conduct, Oil, Mark, Bleed) drives **synergy reactions** — frozen enemies SHATTER into their neighbors, oiled enemies hit by fire FLARE. The player resists incoming elemental damage too (item resist affixes) and can be **afflicted by elemental enemies** — Cryo hits CHILL your thrust, Toxic hits CORRODE you (more damage taken), Pyro hits BURN you (a DoT) (6.75.0–6.76.0). So **Cinder ignites you on contact, Plaguebearer corrodes, Frost Lance chills.** *(Foundation live; elemental weapons + more enemy/player status effects rolling out.)*
...
- **5-slot inventory with rolled rarities (6.0.0; loot-feed rework 6.46.0)** — cockpit / hull (HP), shielding / chassis (toughness), and the **nanites** trinket (regen), with multi-affix rolls scaling across an **8-tier rarity ladder** (6.60.0 — Common 1 → Rare 2 → Exceptional 3 → Legendary 3 → Epic 4 → Godlike 4 → Divine 5 → Transcendental 5 affixes, each tier rarer with a distinct glow color; Transcendental shimmers prismatic) mirroring the passive stats — HP, toughness, vampirism, thorns, crit, dodge, speed, regen — plus **per-element resistance** rolls (6.65.0 — Pyro / Cryo / Volt / Toxic / Void / Radiant) that reduce that element's incoming damage. **6.46.0** — item drops no longer spawn world pickup orbs. Each roll registers into a **left-edge loot feed** of crystalline rarity-tinted cards (distinct per-slot geometry, no halo circle) that persist ~90s and fade to ~20% when the ship or cursor is over the feed region. Drops still **auto-equip if they beat the slot**; ALL drops are kept so the **`I` inventory screen** (6.47.0) can review equipped gear and **re-equip a past drop**.
...
- **Persistent meta leveling (6.35.0)** — kills grant XP (12 regular / 120 boss) toward a player **level** (cap 100), and each level grants **1 SP**. Level / XP / SP / allocations persist across runs in localStorage (not cleared by NEW GAME) — a slow, deliberate "keep coming back" hook (~3-4 levels per early run, tapering hard toward 100). SP is spent in the STATS screen on permanent passive stats: Health, Toughness, Vampirism, Thorns, Crit Chance, Crit Damage, Evasion, Speed (each capped at 20 points = its full value; per-point = max ÷ 20). **(6.36.0)** SP is **freely redistributable** via per-stat **[−] / [+]** buttons, and the STATS screen auto-opens after a wave where the player leveled up.
...
- **Save / Continue** (5.79.0): wave-start auto-save lets the player resume from the title screen's **CONTINUE** button.
- **Persistent cross-run progression (6.49.0)**: a single profile in localStorage (`rainboidsMeta`) carries your **gold, upgrades/passives, equipped gear, and level/XP/SP across every run**. **NEW GAME** starts a fresh session at stage 1-1 with a **random loadout** but **carries all your accumulated gold / items / upgrades**; **CONTINUE** resumes where you left off (with that same accumulated progression).
- **Regular autosave (6.50.0)**: progress saves automatically every ~15s and on tab hide/close (`visibilitychange` / `pagehide`), so closing or resetting never loses anything. Because progression is re-applied from the profile after a CONTINUE, **repeatedly dying on a section keeps accumulating gold / XP / SP from every attempt**.
- **Diablo-style stats screen** (5.79.0): press \` (or the **STATS** button, next to UPGRADES on the bottom bar) to pause and inspect vitals / offense / economy with hover tooltips explaining every formula. **(6.36.0)** Now leads with a **STAT POINTS** card — LEVEL / SP / XP header plus a **[−] points/20 [+]** control per permanent stat for spending and freely redistributing SP. Auto-opens after a level-up wave.
