...
- **Persistent cross-run progression (6.49.0)**: a profile in localStorage (`rainboidsMeta`) carries your **gold, upgrades/passives, equipped gear, and level/XP/SP across every run**. **NEW GAME** keeps all of that and only **re-rolls the loadout** (primary / power / skill) + resets the wave/score — it no longer wipes accumulated progression. The profile saves on each wave-start and on death.
