...
Elements & resistances, rolled rarity-tiered gear, kill-streak power tiers, and a fixed 50-wave boss gauntlet — all in your browser, no install.
...
**🔥 Elements & resistances** — 7 elements (Kinetic · Pyro · Cryo · Volt · Toxic · Void · Radiant), each with a signature status (Burn, Corrode, Chill/Freeze, Conduct, Oil, Mark, Bleed) that **deals extra damage** (on-hit bonus plus a damage-over-time tick — every element bites) and **emits matching particle VFX** off the afflicted enemy: burn flares red-orange, corrosion belches green clouds, freeze wreathes a blue mist (and tints the enemy blue), volt crackles yellow-white, void marks glow purple. Enemies carry weaknesses and immunities, so bringing the right element matters — and statuses **chain into reactions**: frozen enemies SHATTER into neighbors, oiled enemies hit by fire FLARE. Elemental enemies afflict *you* back.
...
**📈 Progression** — a **card draft on every stage clear except the last** (9 drafts across the fixed 50-wave campaign, at boss waves 5/10/…/45) offers **1 primary + 1 power + 2 ability** **efficacy** cards (amplifiers only — more/faster/bigger of what you brought; the build-defining attunements + mods are chosen upfront), all **relevance-filtered** to your equipped loadout — pick one to deepen a carry. Kills grant XP toward a **persistent account level** (cap 100); each level = 1 **SP**, and the **STATS screen auto-opens at stage clear** when you've leveled, to spend it on permanent stats (HP, toughness, crit, dodge, speed, regen, vampirism, thorns, and the power-weapon **energy economy** — capacitor / reactor / efficiency). A **20-tier kill-streak ladder** (EMPOWERED → … → RAINBOIDS GOD) buffs damage until you take a hit.

**🏁 Campaign** — a fixed **50-wave speedrun** structured as **10 blocks × 5 waves** (`1-1` → `10-5`): waves 1-2 are normal, wave 3 an **ELITE/miniboss** wave, wave 4 a light **cooldown** breather, and wave 5 the block's **boss**. Bosses land on waves **5, 10, …, 50** with escalating boss tiers, and rewards climb with depth (the **Reward Dial**: gold/drops/rarity/Cores scale up as you push deeper). An **adaptive difficulty director** quietly tunes the challenge to how you're actually playing — clearing fast earns more/tankier targets, surviving comfortably raises the threat — all behind the scenes, so escalation is felt rather than read off a meter. Every block final is a **unique multi-phase boss** — 10 in all, each with its own gimmick (rotating weak-points, element-gated armor plates, shield-drone reflectors, Pyro/Cryo twins, conduit-node storms, egg-sac swarms, four-element turrets, an adaptive resist wall, a projectile-devouring maw, and a 5-aspect all-element finale at wave 50). The whole run is **timed**, and the **final time** headlines the Game Complete screen alongside accuracy, damage, kills, and your preferred weapon. Because the campaign is a speedrun, a faster clear grades into a higher named completion tier — **CASUAL** (under 20 min) → STEADY → EMPOWERED → UNSTOPPABLE → LEGENDARY → **GODLIKE** (under 5 min). **Meta persists** across runs in `localStorage` — account-gold, unlocks, gear, upgrades, and level/SP carry over: `NEW GAME` opens the **BUILD** screen — a tabbed **bubble tree** (GEAR · PRIMARY · POWER · DEFENSE · PASSIVES · STATS) where you review/manage gear, unlock weapons/abilities/passives with account-gold, pick your **4 primary + 4 power + 4 ability** loadout, equip **Passives** (see below), and set the run **difficulty** in **RUN SETUP** (the campaign is a fixed 50 waves) — then `START RUN`; `CONTINUE` resumes mid-run, skipping the pre-run screen. **New accounts get an on-ramp**: a small **gold head-start** plus a few **starter unlocks** beyond the base kit, so the very first LOADOUT is a real choice (rapid vs. spread vs. pierce primaries) rather than a single forced loadout — and the early waves **front-load enemy variety**, debuting a distinct new threat almost every wave through Stage 3 (seven enemy types and three different bosses by wave 9) so the opening never feels repetitive. The **meta account stays continuously in sync** — gold, gear, level/SP, unlocks, and powerups flush to `localStorage` within ~1s of any change (coalesced so bursts of pickups are a single write) and again on tab close; the mid-run resume bookmark autosaves every ~15s.
...
js/
  ...
  modules/
    ...
    enemy/              # entity, data/archetypes, firing, shapes, boss phases/parts/intro/rage/fx/render, bosses/ (10 unique)
                        #   steering AI: steering.js (Reynolds + momentum), context-steering.js,
                        #   navgrid.js (A*/flow-field for boss arenas), strategy.js + brain.js
    combat/             # elements, collision, weapon-data, combat-manager, matchup-data (weapon↔archetype)
