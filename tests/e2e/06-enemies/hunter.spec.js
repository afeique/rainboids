 ...
 * E2E-06: HUNTER enemy
 *
 * Spawns a single HUNTER, runs the AI playtester until the enemy dies or the
 * timeout elapses, then verifies the enemy pool shrunk.  Also tests that the
 * HUNTER can deal damage to the player when spawned nearby.
 */
...
const ENEMY_TYPE  = 'HUNTER';
const TEST_TIMEOUT = 60_000;

/** Clear all active enemies and asteroids so the test environment is clean. */
...
test.describe(`E2E-06: ${ENEMY_TYPE} enemy`, () => {
    ...
    test(`AI can kill ${ENEMY_TYPE} within 20 seconds`, async ({ page }) => {
        // Enable one-punch-man cheat to guarantee kill speed
        await page.evaluate((type) => {
            const ge = window.gameEngine;
        ...
        const enemyCountBefore = await page.evaluate(
            () => window.gameEngine.enemyPool.activeObjects.length
        );
    ...
    test(`${ENEMY_TYPE} deals damage to player when spawned nearby`, async ({ page }) => {
        ...
        // Spawn enemy directly on the player — collision fires immediately
        await page.evaluate((type) => {
            const ge = window.gameEngine;
    ...
    test(`${ENEMY_TYPE} pool count decreases after enemy is released`, async ({ page }) => {
        await page.evaluate((type) => {
            const ge = window.gameEngine;
            ge.enemyPool.get(ge.gameField.width / 2, 200, type, 1);
        }, ENEMY_TYPE);
        ...
        await page.evaluate(() => {
            const ge = window.gameEngine;
            if (ge.enemyPool.activeObjects.length) {
