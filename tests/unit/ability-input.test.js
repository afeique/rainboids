/**
 * tests/unit/ability-input.test.js — Phase B.S2 keyboard ability-slot binding.
 *
 * Pins the number-key → ability-slot contract for InputHandler:
 *   - Digit1..4 / Numpad1..4 raise a one-shot per-slot pulse
 *     (input.activateAbilitySlot[0..3]).
 *   - Auto-repeat (e.repeat=true) does NOT raise the pulse, so holding a
 *     number key can't spam re-activations.
 *   - The retired TAB binding no longer raises any ability pulse.
 *
...
describe('B.S2 — number keys 1–4 activate ability slots', () => {
    let handler;
    beforeEach(() => {
        handler = new InputHandler();
    });

    test('input.activateAbilitySlot initializes to four cleared pulses', () => {
        expect(handler.input.activateAbilitySlot).toEqual([false, false, false, false]);
    });

    test('Digit1..4 each raise the matching slot pulse (Digit1→0 … Digit4→3)', () => {
        handler.handleKeyDown(keydown('Digit1'));
        expect(handler.input.activateAbilitySlot).toEqual([true, false, false, false]);

        handler.handleKeyDown(keydown('Digit2'));
        expect(handler.input.activateAbilitySlot).toEqual([true, true, false, false]);

        handler.handleKeyDown(keydown('Digit3'));
        expect(handler.input.activateAbilitySlot).toEqual([true, true, true, false]);

        handler.handleKeyDown(keydown('Digit4'));
        expect(handler.input.activateAbilitySlot).toEqual([true, true, true, true]);
    });

    test('Numpad1..4 mirror Digit1..4 slot mapping', () => {
        handler.handleKeyDown(keydown('Numpad1'));
        handler.handleKeyDown(keydown('Numpad4'));
        expect(handler.input.activateAbilitySlot).toEqual([true, false, false, true]);
    });
    ...
    test('Tab keydown does NOT raise any ability pulse (TAB binding retired)', () => {
        handler.handleKeyDown(keydown('Tab'));
        expect(handler.input.activateAbilitySlot).toEqual([false, false, false, false]);
        // Legacy slot-0 flag (kept for the gamepad path) must also stay clear.
        expect(handler.input.activateAbility).toBe(false);
    });

    test('KeyQ keydown does NOT raise any ability pulse (Q binding retired)', () => {
