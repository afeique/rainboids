...
// ═════════════════════════════════════════════════════════════════════
// Player-vs-drop pickup (Phase 2.5 — dispatch 6).
// ═════════════════════════════════════════════════════════════════════
//
// `detectPlayerDropPickups` is the sixth pure-step pair, extracted from
// the player-collectible loop in `handleCollisions`
// (`js/modules/combat/collision-system.js`, lines 333-491). This pair
// is fundamentally DIFFERENT from every collision pair above:
//
//   - It's a PICKUP, not a physical collision. NO impulse, NO separation,
//     NO damage to either side. The "hit" IS the consumption.
//   - The wrapper's downstream behavior is DROP-KIND dependent: health
//     restores HP, money-shape / money-pixel add coins (with upgrade
//     multipliers), powerup grants the powerup. The pure step does NOT
//     know any of that — it only reports the geometric overlap and the
//     drop's discriminator + raw `value`.
//   - NO piercing — every drop is single-pickup.
//   - NO magnet / tractor logic here. Those are in `js/sim/drops.js`.
//     By the time this scan runs, drops have already moved toward the
//     player via magnetism. We only test the final overlap.
//
// Event shape: exactly 6 non-type fields (playerId, dropId, dropKind,
// value, dropX, dropY). Total 7 keys with `type`. NO damage / impulse
// / separation / piercing fields.
//
// The tests below pin:
//   - single pickup happy path (all event fields present, correct)
//   - geometry miss (no event)
//   - multiple drops picked up in one tick (multiple events)
//   - skip gates (inactive player, inactive drop)
//   - each drop kind passes through correctly (health, money_shape,
//     money_pixel, powerup)
//   - `value` propagates verbatim (no upgrade multipliers applied
//     pure-side)
//   - defensive empty / null inputs

// ---------------------------------------------------------------------
// Helpers — make a player with a radius, and a drop at (x, y) using
// `freshDropState`. Drops default to radius 14, value 1 for health and
// 5 otherwise (see freshDropState defaults).
// ---------------------------------------------------------------------
...
// ---------------------------------------------------------------------
// Test — single pickup happy path. The event should contain exactly 6
// non-type fields with the expected values.
// ---------------------------------------------------------------------
