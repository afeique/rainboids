...
//! `RoomState.bullets` happens in `room.rs` Wave 2 integration —
...
/// Bullet velocity multiplier vs the sim base `BULLET_SPEED` (8.0
/// px/tick). Solo's `bulletSpeed: 1.1`.
pub const BULLET_SPEED_MULT: f64 = 1.1;

/// Bullet lifetime multiplier vs the sim base
...
/// Wave 2 integration in `room.rs` maps these to `BulletState`
...
///
/// Caller (Wave 2 integration in `room.rs::process_fire_inputs`)
/// is responsible for:
