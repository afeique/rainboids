...
pub enum ClientMsg {
    ...
    Input {
        ...
        /// `sim::weapon::fire(weapon, ship, rng)`. See
        /// `sim::weapon::KIND_*` for the dense u8 enumeration.
...
pub enum EventPayload {
    ...
    /// `sim::enemy::spawn_hunter_from_seed`. Both sides produce the
    /// same enemy.
    EnemySpawn {
        enemy_id: u32,
        kind: u8,
    ...
    /// via `sim::asteroid::spawn_from_seed`. Used at room boot
    ...
    /// `sim::drops::spawn_orb_from_seed` so both sides produce the
