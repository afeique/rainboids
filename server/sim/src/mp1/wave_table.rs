...
//! `mp1_room.rs` substitutes HUNTER for unimplemented kinds.
//!
...
//! `WaveConfig` / `WaveSpawnGroup` from `crate::mp1::wave` — when
...
use crate::mp1::wave::{WaveConfig, WaveSpawnGroup};

// ─── Enemy kind constants (dense 0..=9) ──────────────────────────────
// Only HUNTER has an implemented spawn function in mp1 today; the
// other 9 are reserved for Phase 4 step 5. The wave table emits them
// anyway so the table is authoritative; `mp1_room.rs` substitutes
// HUNTER for unimplemented kinds with a debug log until step 5.

/// HUNTER — chase-the-player. Implemented in `mp1::enemy::spawn_hunter_from_seed`.
