...
//! site (which enemy died → which orbs drop) is owned by `mp1_room.rs`
...
//!     tractor skill isn't implemented in mp1 yet (Phase 5 follow-up),
//!     so gold orbs in mp1 drift freely until contact pickup.
...
pub fn update_orb(orb: &mut OrbState, ships: &[ShipState], field_w: f64, field_h: f64) {
    ...
    // 246-255 implement a tractor scoop for gold orbs; mp1 will
