...
//
// Hand-mirrored from `server/src/protocol/mod.rs` — every variant must
// match name, declaration order (tag value), and field layout. The
// schema source-of-truth is `schema/protocol.toml`; v2 will codegen
// this module from it.
//
// Tag rule: bincode 1.x encodes serde-derived enums as `u32` variant
// indices in the order they appear in the Rust enum. Reordering or
// inserting a variant in the middle is a wire-breaking change — always
// append.
//
// Newtype rule: tuple structs with one field (e.g. `PlayerId(u64)`)
// serialize transparently as the inner type. So PlayerId/RoomId/etc.
// are 8 bytes (u64), and PowerupId is 2 bytes (u16).
...
/* ─── ClientMsg / ServerMsg / GameEvent variant tags ──────────────────────── */
...
/** PackedInput: i8 i8 i16 i16 u8 (7 bytes). */
export function writePackedInput(w, p) {
    w.i8(p.moveX);
    w.i8(p.moveY);
    w.i16(p.aimX);
    w.i16(p.aimY);
...
/** PeerInfo. */
export function writePeerInfo(w, p) {
    w.u64(p.playerId);
    w.str(p.displayName);
    w.u8(p.slot);
}
...
/** EnemyState. */
export function writeEnemyState(w, e) {
    w.u64(e.id);
    w.u8(e.kind);
    w.f32(e.x);
    w.f32(e.y);
...
/** AsteroidState. */
export function writeAsteroidState(w, a) {
    w.u64(a.id);
    w.u8(a.size);
    w.f32(a.x);
    w.f32(a.y);
...
/** DropState. */
export function writeDropState(w, d) {
    w.u64(d.id);
    w.u8(d.kind);
    w.f32(d.x);
    w.f32(d.y);
...
/** SnapshotPayload: 4 vecs of states. */
export function writeSnapshotPayload(w, p) {
    w.vec(p.ships, writeShipState);
    w.vec(p.enemies, writeEnemyState);
    w.vec(p.asteroids, writeAsteroidState);
    w.vec(p.drops, writeDropState);
}
...
/** EntityRef: tagged enum, each variant has a single u64 id payload. */
...
export function writeGameEvent(w, ev) {
    switch (ev.type) {
        case EVT.BULLET_SPAWN:
            w.variant(EVT.BULLET_SPAWN);
            w.u64(ev.id);
            w.u64(ev.owner);
            w.u32(ev.weapon); // WeaponId enum tag
            w.f32(ev.x);
            w.f32(ev.y);
            w.f32(ev.vx);
            w.f32(ev.vy);
        ...
        case EVT.BULLET_DESPAWN:
            ...
            w.u64(ev.id);
            w.u32(ev.reason); // DespawnReason
            return;
        case EVT.ENEMY_DESTROY:
            w.variant(EVT.ENEMY_DESTROY);
            w.u64(ev.id);
            w.option(ev.by, (ww, v) => ww.u64(v));
            w.vec(ev.drops, (ww, v) => ww.u64(v));
            return;
        case EVT.ASTEROID_DESTROY:
            w.variant(EVT.ASTEROID_DESTROY);
            w.u64(ev.id);
            w.option(ev.by, (ww, v) => ww.u64(v));
            w.vec(ev.fragments, (ww, v) => ww.u64(v));
            return;
        ...
        case EVT.DAMAGE_NUMBER:
            ...
            w.f32(ev.y);
            w.i32(ev.value);
            w.u32(ev.kind); // DmgKind
            return;
        default:
            throw new TypeError(`writeGameEvent: unknown variant ${ev.type}`);
    }
}
...
export function readGameEvent(r) {
    ...
    switch (tag) {
        ...
        case EVT.BULLET_DESPAWN:
            return { type: tag, id: r.u64(), reason: r.u32() };
        case EVT.ENEMY_DESTROY:
            return {
                type: tag,
                id: r.u64(),
        ...
        case EVT.ORB_COLLECT:
            return { type: tag, id: r.u64(), by: r.u64(), value: r.u32() };
        case EVT.PLAYER_DAMAGED:
            return { type: tag, player: r.u64(), hp: r.f32() };
        case EVT.PLAYER_DOWNED:
            return { type: tag, player: r.u64() };
        case EVT.PLAYER_REVIVED:
            return { type: tag, player: r.u64(), by: r.u64() };
        case EVT.WAVE_START:
            return { type: tag, wave: r.u32(), enemyCount: r.u32() };
        case EVT.WAVE_CLEAR:
            return { type: tag, wave: r.u32(), timeMs: r.u32() };
        case EVT.POWERUP_OFFER:
            return { type: tag, player: r.u64(), picks: r.u8() };
        case EVT.POWERUP_CHOSEN:
            return { type: tag, player: r.u64(), powerup: r.u16() };
        case EVT.HIT_FLASH:
            return { type: tag, entity: readEntityRef(r), intensity: r.f32() };
        case EVT.DAMAGE_NUMBER:
            return {
                type: tag,
                x: r.f32(),
...
export function readClientMsg(r) {
    ...
    switch (tag) {
        ...
        case C2S.JOIN_ROOM:
            return { type: tag, roomId: r.u64() };
        case C2S.JOIN_ROOM_BY_CODE:
            return { type: tag, code: r.str() };
        case C2S.LEAVE_ROOM:
            return { type: tag };
        case C2S.INPUT:
            return { type: tag, tick: r.u32(), packed: readPackedInput(r) };
        case C2S.ACK:
            return { type: tag, snapshotTick: r.u32() };
        case C2S.PONG:
            return { type: tag, clientT: r.u32(), serverT: r.u32() };
        case C2S.POWERUP_CHOOSE:
            return { type: tag, powerup: r.u16() };
        case C2S.REVIVE:
            return { type: tag, target: r.u64() };
        case C2S.CHAT:
            return { type: tag, text: r.str() };
        default:
...
export function readServerMsg(r) {
    ...
    switch (tag) {
        ...
        case S2C.ERROR:
            return { type: tag, code: r.u32(), msg: r.str() };
        case S2C.ROOM_LIST:
            return { type: tag, rooms: r.vec(readRoomSummary) };
        case S2C.ROOM_JOINED:
            return {
                type: tag,
                roomId: r.u64(),
        ...
        case S2C.ROOM_LEFT:
            return { type: tag, reason: r.u32() };
        case S2C.PEER_JOINED:
            return { type: tag, peer: readPeerInfo(r), slot: r.u8() };
        case S2C.PEER_LEFT:
            return { type: tag, slot: r.u8(), reason: r.u32() };
        case S2C.SNAPSHOT:
            return {
                type: tag,
                tick: r.u32(),
        ...
        case S2C.EVENT:
            return { type: tag, tick: r.u32(), event: readGameEvent(r) };
        case S2C.PING:
            return { type: tag, clientT: r.u32(), serverT: r.u32() };
        default:
