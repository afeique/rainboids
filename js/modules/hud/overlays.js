...
export function drawStreakIndicator() {
    ...
    // Show from the first kill so the idle-countdown (land a kill before it
    // resets) is visible the whole time a streak runs. `_streakDrawn` is a
    // queryable QA marker (the kill count when shown, else null).
    ...
    let currentTier = null, currentIdx = -1;
    ...
    // The BIG line: the tier word (EMPOWERED … GODLIKE … RAINBOIDS GOD), or
    // "STREAK" before the first tier (10 kills).
    ...
    // ── MOBILE — top-center, rank LARGE + count small, no bars (see-through). ──
    if (isMobile()) {
        ...
        ctx.lineWidth = 3;
        ctx.strokeText(`${k} KILL${k === 1 ? '' : 'S'}`, x, y + 26);
        ctx.fillText(`${k} KILL${k === 1 ? '' : 'S'}`, x, y + 26);
        ctx.restore();
        return;
    }

    // ── DESKTOP — TOP-RIGHT (out of the play area). Rank word LARGE, kill
    //    count small beneath it, then the gold-find readout + tier-progress +
    //    idle-countdown bars, all right-aligned. ──
    ...
    // Kill count (small) beneath the rank word.
    ctx.font = "bold 11px 'Press Start 2P', monospace";
    ctx.fillStyle = 'rgba(255, 255, 255, 0.9)';
    ctx.lineWidth = 3;
    ctx.strokeText(`${k} KILL${k === 1 ? '' : 'S'}`, RIGHT, y + 26);
    ctx.fillText(`${k} KILL${k === 1 ? '' : 'S'}`, RIGHT, y + 26);
    ...
    if (pct > 0) {
        ...
        ctx.fillStyle = '#ffd700';
        ctx.lineWidth = 2;
        ctx.strokeText(`+${pct}% GOLD`, RIGHT, by);
        ctx.fillText(`+${pct}% GOLD`, RIGHT, by);
        ctx.lineWidth = 3;
        ctx.strokeStyle = '#000000';
        by += 14;
    }
    ...
    // Idle-countdown bar (green→red drain over 10s — land a kill before empty).
    {
