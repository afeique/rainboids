...
export class GameEngine {
    ...
    // 8.9.0 — the run stage DRAFT (the "choose your next stage" risk/reward
    // picker) was removed: waves are purely CPU-governed now, so a stage clear
    // advances straight through with no choice moment. (openStageDraft /
    // _applyStageSpec / DraftOverlay are gone; the income faucet's per-stage R$
    // multiplier reads a null game.runStage and falls back to ×1.)
    ...
    // ENMY-04 — DEBUG HOOK. Force-spawn a PRISM_MIRROR (front-arc reflector) on
    // demand for QA + manual testing, mirroring spawnNullDrone. Spawns ~260px off
    // the player (or viewport center), no warp, instantly fightable. Returns the
    // enemy (carrying a `reflect` config + `reflects` flag), or null if the pool
    // is dry.
    //   gameEngine.spawnPrismMirror()  or  gameEngine.spawnPrismMirror({ x, y })
    spawnPrismMirror(opts = {}) {
        // Default to ~260px off the player (or viewport center) so the freshly
        // spawned mirror doesn't immediately ram the ship and die before QA can
        // observe its reflect arc. Explicit opts.x/opts.y override.
        const baseX = this.player ? this.player.x
            : (this.camera ? this.camera.x + this.width / 2 : (this.gameField ? this.gameField.width / 2 : 600));
        const baseY = this.player ? this.player.y
            : (this.camera ? this.camera.y + this.height / 2 : (this.gameField ? this.gameField.height / 2 : 400));
