...
| RUN-01b | `wave-manager.js` + `combat/card-draft.js` | ⏳ **REMAINDER** — economy/cadence: boss=last/stage (done) + **elite=other mult-of-3** injection + **card every stage EXCEPT last (cards=stages−1)**. Player-facing — own QA *(X1 cont.)* | RUN-01a | — |
