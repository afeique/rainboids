...
| P6 | Catalog **batch 1** (~10–12, one per archetype…) — each w/ a live consumer at its hook + unit test | Each shipped passive measurably changes play; at least 3 synergy clusters demonstrably stack; unit tests | P2 | 🟡 IN PROGRESS — full **~46-entry catalog data shipped** (§10 + §10.1, 6.125.0). **Effects batch 1 LIVE (6.125.0):** Glass Cannon (+60% dmg via `getPassiveDamageMult` on `applyDamageToEnemy` + −50% maxHP via `getPassiveMaxHpMult`), Opportunist (+15% vs status-afflicted), Last Bastion (+20% dodge <30% HP). Data-driven `damageMult`/`maxHpMult` fields. **Batch 2 LIVE (6.126.0):** Predator (first-hit-on-full-HP crit), Vampiric Rounds (crits heal 2 HP), Hoarder's Greed (+100% gold via `getGoldFindMultiplier` / +15% dmg-taken). **Batch 3 LIVE (6.127.0):** Overflow Spark (+25% primary at full energy), Failsafe (per-hit cap 50% maxHP + −15% maxHP), Second Heart (lethal save once/stage @30% HP). **Batch 4 LIVE (6.128.0):** Scavenger (+50% gear drop rate), Purist (+40% dmg / no crit). **Batch 5 LIVE (6.129.0):** Prismatic Soul (bullet element cycle), Catalyst (+50% reactions + extra shatter chain). **Batch 6 LIVE (6.130.0):** Killing Spree (×2 streak damage bonus). **Batch 7 LIVE (6.133.0):** Gunslinger (+50% primary dmg / +30% fire rate, disables power+abilities). **Batch 8 LIVE (6.134.0):** Hex Touch (+20% burn/bleed DoT). **Batch 9 LIVE (6.135.0):** Frostbite (freeze threshold −25%), Static Charge (every-5th-hit conduct zap). **Batch 10 LIVE (6.136.0):** Overkill (kill-excess splash to nearest), Ricochet (kill bounces 50% shot to nearest) — shared `_passiveKillSplash` one-hop helper. **Batch 11 LIVE (6.137.0):** Detonator (status-afflicted kill → 110px AoE burst + status spread, `_detonateStatuses`). **Batch 12 LIVE (6.138.0):** Harvest (status-DoT-kill → bonus energy + gold orb via `harvestBonus` engine helper at the enemy.js DoT-death branch). **Batch 13 LIVE (6.139.0):** Guardian Echo (≤25%-HP threshold-crossing hit → 200px knockback nova, `guardianEchoNova`; no death-save). **Batch 14 LIVE (6.140.0):** Tracer Lock (same-target hit ramp +8%/stack, cap +40%, resets on swap; pure `tracerLockStep` on `applyDamageToEnemy`). **Batch 15 LIVE (6.141.0):** Vendetta (last attacker becomes grudge target → +30% via `vendettaMult` on `applyDamageToEnemy`; `_vendettaTarget` stamped in takeDamage). **Batch 16 LIVE (6.142.0):** Flow State (each kill cuts every ability's remaining cooldown by 3% of its max via pure `flowStateReduce` from `onEnemyKill`). **Batch 17 LIVE (6.143.0):** Kindling (burn/corrode spreads to +1 nearest fresh enemy ≤120px; pure `kindlingTarget` + `_spread` re-entry flag in `applyBurn`/`applyCorrode`). **Batch 18 LIVE (6.144.0):** Resonance (every 3rd power-weapon use costs no energy; pure `resonanceStep` counter at both power-fire deduct sites in weapons.js). **Batch 19 LIVE (6.145.0):** Kinetic Battery (successful dash refunds 20 power energy; guarded refund in `Player._triggerDash`, `KINETIC_BATTERY_REFUND` const). **Batch 20 LIVE (6.146.0):** Conduit (statuses tick 25% faster but expire 25% sooner — duration ×0.75 at all combat-manager apply* sites + burn/bleed tick-interval ×0.75 in `Enemy._processStatusEffects`; tick count preserved; pure `conduitFactor`). **Batch 21 LIVE (6.147.0):** Backlash (a dodge retaliates with a 12-dmg strike at the attacker — bullet→shooter, else→source; pure `backlashTarget` + guarded `damageEnemy` on the dodge-success path). **Batch 22 LIVE (6.150.0):** Purist "shots pierce" (+1 bullet piercing via `puristPierceBonus` in `applyGlobalBulletUpgrades`) + Scavenger "huge pickup radius" (+70px in `starCollision`, `SCAVENGER_PICKUP_BONUS`) — completing both keystones' full descriptions. **Batch 23 LIVE (6.151.0):** Frenzy keystone (+8% dmg per nearby enemy, cap +80%, via on-demand `frenzyNearbyCount`+`frenzyMult` at `applyDamageToEnemy`; +30% damage-taken downside in `takeDamage` next to Hoarder's). **Batch 24 LIVE (6.152.0):** Siege (standing-still damage ramp to +60% over ~2s, decays in ~0.6s on the move; `_siegeRamp` advanced in `player.update()` from speed, applied via `getSiegeDamageMult()` at `applyDamageToEnemy`; pure `siegeRampStep`/`siegeMult`). **Batch 25 LIVE (6.153.0):** Gravity Well keystone — constant weak pull (0.7px/frame, ≤420px, bosses exempt) of enemies toward the reticle, run each frame via engine `applyGravityWell` (guarded) using pure overshoot-capped `gravityWellPull`. **Batch 25 LIVE (6.153.0):** Gravity Well. **Batch 26 LIVE (6.154.0):** Eye of the Storm keystone — while player ~stationary (speed ≤0.5), slow nearby enemies (≤300px) 40% via refresh-style `applySlow` + one-time ×0.6 velocity damp on enemy bullets entering the radius (`_eyeSlowed`, cleared in EnemyBullet.reset); engine `applyEyeOfTheStorm` called each frame (guarded). **Batch 27 LIVE (6.155.0):** Afterimage — a dash fires the primary once from the dash ORIGIN (at `_triggerDash` time this.x/this.y is still the origin; `firePrimary` aims along this.angle; pools from `this.gameEngine`); free bonus shot, guarded no-op. **Batch 28 LIVE (6.156.0):** Twin Cast keystone — power weapons fire twice (2nd @50%) for +30% energy. +30% at BOTH energy-deduct paths (composes w/ Resonance); double-fire re-dispatches the 6 BURST powers (Nova/Missiles/Singularity/Cryo/Orbital/Mine) with a half-damage CLONE (`twinCastHalfConfig` scales each power's distinct damage field on a copy — shared POWER_WEAPONS never mutated); beam/buff powers don't double; "+1 ability charge" clause DEFERRED (abilities use cooldowns, not charges — needs new machinery). Pure `twinCastEnergyCost`/`twinCastDoubles`/`twinCastHalfConfig`. **✅✅✅ ENTIRE PHASE P6 PASSIVE CATALOG NOW WIRED — every catalog entry has a live, unit-tested, QA-verified gameplay effect (37 effects + 2 secondary clauses; 96 unit suites / 902 tests).** P6 COMPLETE. Remaining P-phase polish: P5b in-run pause-menu PASSIVES swap UI was shipped earlier; Phase X (difficulty system, user redesigning §13) remains the separate big next feature — PARKED unless user directs. Only follow-up nit: Twin Cast "+1 ability charge" (needs an ability-charge system). |
