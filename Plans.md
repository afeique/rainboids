...
| CD-02 ◐ | `combat/passive-data.js` | ~~add Bloodshield/Bloodlust/Sanguine/Hemoglutton~~ ✅ (6.199–6.201). **No-downsides: B1 ✅ 6.208.0** (HOARDERS/FRENZY +taken, TWIN_CAST +cost) · **B2 ✅ 6.209.0** (GUNSLINGER no-powers→+primary anchor; PURIST no-crit→+dmg+pierce) · **B3 ✅ 6.210.0** (FAILSAFE −15%maxHP dropped→per-hit-cap as pure upside; EYE_OF_THE_STORM flavor downside removed). **4 of 11 remain.** **STILL TODO (later batches — these need MECHANIC reworks, not just deletions):** GLASS_CANNON+BERSERKERS merge→"+40%→+90% as HP falls" (frees slot)→add **Apex Predator** (HP-scaling dmg); HEAT_SINK (drop lockout→vent AoE); GRAVITY_WELL ("pulls danger toward you" — verify mechanic); OVERFLOW_CAPACITOR (×1.5 cost→pure 2×regen/+50%max — verify energy wiring vs the 6.197.0 getters); Overclock/Capacitor-Bank passives; `synergies:[ids]` metadata. **⚠ then enemy-HP re-tune + extremes playtest (§7) — USER.** |
