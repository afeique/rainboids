...
|------|------|-----|---------|--------|
| A.E1 | Element taxonomy + data model: NEW `combat/elements.js` (ELEMENTS config + `elementalMultiplier` helper); `element` field on all 22 weapons, 10 enemies, and bullets; resist maps | Unit test for `elementalMultiplier` (neutral/weak/resist/immune) green; all weapons+enemies carry element+resist; AI survival run still passes (zero gameplay change) | - | cc:WIP |
| A.E2 | Player→enemy resistance wiring: `elementalMultiplier(enemy.resist, element)` into `applyDamageToEnemy`; resisted/weak/IMMUNE damage-number cues | Unit test: damage scales by resist map; Volt weapon visibly does more vs Volt-weak, 0 vs immune | A.E1 | cc:TODO |
