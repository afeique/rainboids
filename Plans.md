...
| ~~AS-5~~ ✅ core | `game-engine.js`, `ui-manager.js` | **DONE 6.220.0 (core)** — ASSISTS tab reachable on touch (was gamepad-only); touch now honors saved LEVEL too (dodge/aggression already did); inert aim/fire/cast toggles hidden on touch, leaving Level/Auto-Dodge/Aggression. Aim/fire floored in `player.js` → playable at any setting. Desktop unchanged (QA 6/6). **Deferred (device-gated):** on-touch auto-cast tunability (needs mobile-default redesign), stick-side toggle, touch-screen feel. |
| ~~AS-6~~ ✅ logic | `assist/assist-system.js` | **DONE 6.221.0 (logic)** — pure `pickSmartCast` (higher-scored ability vs power, auto-cast forced on so it works at MANUAL level) + `AssistSystem.smartCast(input)` applies it like `act()`. 5 unit. **Deferred (device-gated):** the default-hidden HUD button that calls `smartCast()` — placement/feel wants a real device. |
