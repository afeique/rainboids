...
| CD-02 ◐ | `combat/passive-data.js` | ~~add Bloodshield/Bloodlust/Sanguine/Hemoglutton~~ ✅ (6.199–6.201). **No-downsides: B1 ✅ 6.208.0** (HOARDERS/FRENZY +taken, TWIN_CAST +cost) · **B2 ✅ 6.209.0** (GUNSLINGER, PURIST) · **B3 ✅ 6.210.0** (FAILSAFE, EYE_OF_THE_STORM) · **B4 ✅ 6.211.0** (OVERFLOW_CAPACITOR reworked to pure 2×regen/+50%max + WIRED into the energy getters; GRAVITY_WELL flavor downside removed). **2 of 11 remain.** **STILL TODO (net-new-mechanic batches):** GLASS_CANNON+BERSERKERS merge→"+40%→+90% as HP falls" (frees slot)→add **Apex Predator** (HP-scaling dmg — the headline); HEAT_SINK (drop lockout→uncapped fire-rate ramp + vent AoE). Plus Overclock/Capacitor-Bank passives; `synergies:[ids]` metadata. **⚠ then enemy-HP re-tune + extremes playtest (§7) — USER.** |
