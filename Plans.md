...
> 🎉 **ALL 7 ENMY ENABLING-SYSTEM HELPERS NOW LIVE** (telegraph/cloak/reflect/projectile-absorb/buff-strip/suppress-aura/blink-burrow), each wired default-safe (gated on an `enemy.*` flag) + demonstrated by a debug-spawnable archetype (Phantom/Wraithworm/Null-Drone/Prism-Mirror/Devourer/Leech). **NEXT for ENMY:** add the new types to the **WAVE_DATA roster** (so they appear in live waves — balance-touching, the live director absorbs it) + per-type AI flourishes (ENMY-11/12) + remaining types (Beacon/Juggernaut/Thornback/artillery).
