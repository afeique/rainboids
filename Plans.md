...
| ENMY-09 | `enemy/enemy-data.js` + AI | Batch-4 types: Devourer/Phantom/Prism-Mirror/Beacon *(A.E8d)* | ENMY-02/03/04 | 🟡 **PARTIAL** — **PHANTOM 6.180.0** (cloak) + **PRISM_MIRROR 6.183.0** (reflect) + **DEVOURER 6.184.0** (projectile-absorb LIVE: `routeBulletToAbsorb` bullet pre-pass + `consumeAbsorbShield` soak in `applyDamageToEnemy`, maw tracks player, beams/rear bypass, gated `enemy.eatsProjectiles`, `spawnDevourer`). **TODO:** Beacon + add all to WAVE_DATA roster |
| ENMY-10 | `enemy/enemy-data.js` + AI | Batch-5: Leech/Null-Drone/Juggernaut/Thornback/Wraithworm + artillery *(A.E8e)* | ENMY-01/05/06/07 | 🟡 **PARTIAL** — **WRAITHWORM 6.181.0** (blink-burrow+telegraph) + **NULL_DRONE 6.182.0** (suppress-aura) + **LEECH 6.185.0** (buff-strip LIVE: `applyBuffStrip` in player-contact path on live `powerups` Map + `isBuffSuppressed` re-grant guard in `addPowerup` + per-Leech cooldown, gated `enemy.stripsBuff`, `spawnLeech`). **TODO:** Juggernaut/Thornback + artillery + add all to WAVE_DATA roster |

> 🎉 **ALL 7 ENMY ENABLING-SYSTEM HELPERS LIVE** (telegraph/cloak/reflect/projectile-absorb/buff-strip/suppress-aura/blink-burrow), default-safe gated, + 6 archetypes. ✅ **ROSTER LIVE 6.187.0** — the 6 new types now spawn in non-boss mid/late `WAVE_DATA` waves (LEECH/WRAITHWORM/NULL_DRONE/PHANTOM/DEVOURER/PRISM_MIRROR, conservative counts; boss + teaching waves untouched; director absorbs). **NEXT for ENMY:** per-type AI flourishes (ENMY-11/12) + remaining types (Beacon/Juggernaut/Thornback/artillery). **PRE-EXISTING:** 3 stale `tests/e2e/08-waves.spec.js` wave-1 "asteroids-only" assertions fail (wave 1 has had enemies for many versions; not a regression — candidate cleanup).
...
| ~~META-04~~ | `world/item-system.js` + `cores.js` + ARMORY | ✅ **6.186.0** — `rerollItemPassive` (reroll the gear `item.passive`, eligibility-gated, different-id-preferred, roll-onto-none) + `passiveRerollCost`/`canAffordPassiveReroll` + `salvageValue` PASSIVE_SALVAGE_BONUS + ARMORY REROLL PASSIVE row. (NOTE: `item.traits` array is vestigial — the "trait" is `item.passive`.) 14 unit + 3 QA | — |
