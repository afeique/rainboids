...
| CD-02 ◐ | `combat/passive-data.js` | No-downsides: **B1** 6.208.0 (HOARDERS/FRENZY/TWIN_CAST) · **B2** 6.209.0 (GUNSLINGER/PURIST) · **B3** 6.210.0 (FAILSAFE/EYE_OF_THE_STORM) · **B4** 6.211.0 (OVERFLOW_CAPACITOR wired/GRAVITY_WELL) · **B5 ✅ 6.212.0** (GLASS_CANNON↔BERSERKER merge→HP-scaling +40%→+90%, −50%maxHP dropped, Berserker removed). **Only HEAT_SINK's downside remains (1 of 11).** **STILL TODO:** add **Apex Predator** keystone (execute <15% HP — the freed slot, ADDITIVE new keystone); HEAT_SINK rework (drop lockout→uncapped fire-rate ramp + vent AoE — net-new HEAT mechanic); Overclock/Capacitor-Bank passives; `synergies:[ids]` metadata. **⚠ then enemy-HP re-tune + extremes playtest (§7) — USER.** |
