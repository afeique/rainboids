...
| CD-02 ◐ | `combat/passive-data.js` | ~~add Bloodshield/Bloodlust/Sanguine/Hemoglutton~~ ✅ (6.199–6.201). **No-downsides BATCH 1 ✅ 6.208.0** (HOARDERS/FRENZY +taken, TWIN_CAST +cost). **BATCH 2 ✅ 6.209.0** (GUNSLINGER no-powers/abilities→pure +primary anchor; PURIST no-crit→pure +dmg+pierce). **6 of 11 downsides remain.** **STILL TODO (later batches, re-anchor per §6c):** GLASS_CANNON+BERSERKERS merge→"+40%→+90% as HP falls" (frees slot)→add **Apex Predator** (most involved — HP-scaling dmg); HEAT_SINK (drop lockout→vent AoE); FAILSAFE (→system per-hit cap); EYE_OF_THE_STORM/GRAVITY_WELL/OVERFLOW_CAPACITOR reworks; Overclock/Capacitor-Bank passives; `synergies:[ids]` metadata. **⚠ then enemy-HP re-tune + extremes playtest (§7) — USER.** |
