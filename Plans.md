...
| CD-04 ◐ | `world/powerup.js` | ~~Regenerator~~ ✅ 6.202.0 · ~~Life-on-Kill~~ ✅ 6.202.0 · ~~Ablative Plating~~ ✅ 6.203.0 · ~~Surge Battery (first power/wave free)~~ ✅ **6.205.0** · ~~Flux (power cast→regen ramp)~~ ✅ **6.205.0** (R1-mechanical 6.204.0 made these meaningful — power weapons now crit). **STILL TODO:** Overflow-Discharge (full energy→next power free+40%), Resonant-Surge (status applied→+energy — multi-site status hook); Reinforced-Hull (overlaps Bloodshield, reconsider); remove downsides; consolidate 7 heal micro-mods→1–2; `synergies` |
