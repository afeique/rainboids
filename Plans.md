...
- [ ] **T32** 🔴 Run randomizer + draft hook into wave flow [T14,T40]
  > **▶ T32 MAP (surveyed 2026-05-27):** The draft is the STAGE-clear choose-moment that replaces the removed card draft (T22). **Hook point:** `wave-manager.js` wave-clear flow L224-322 — stage clear = `isStageClear(clearedWave, runWavesPerStage(this.game))` (L228); `fireSurvivorOverlay=false` (L295) is where the old card menu was. **Modules (built, NOT yet wired anywhere):** `wave/run-randomizer.js` → `nextDraft(depth, runState={}, rng)` → options[], `applyPick(option)`, `PUNISHING_MODIFIERS` set. `ui/draft-overlay.js` → `class DraftOverlay`, `open(options, {pwr, onPick})` (onPick(index)→close), `isOpen()`/`close()`. **PLAN:** (1) game-engine owns a `DraftOverlay` instance + a `runState` (depth/picked-stages) + `openStageDraft()` — at stage clear, compute `nextDraft(stageIndex, runState)`, `open(options,{pwr:game.playerPWR, onPick})`; onPick → `applyPick(option)` → store the chosen stage config (theme/modifier/composition) on `game.runStage`. (2) wave-manager stage-clear triggers `openStageDraft()` (gate the WAVE_TRANSITION→next-stage advance on the pick, like the old card menu paused it). (3) **Apply the pick:** the chosen theme/modifier/composition must bias the NEXT stage's spawns + difficulty — wire `game.runStage` into the spawn/role config + the director context (STAGE_MODIFIERS/STAGE_THEMES from run-templates T09). (4) PWR-vs-threat readout: overlay shows each option's threat next to `game.playerPWR`. Constants: stage count / draft cadence (2-3 stage choices). **Test:** draft opens at stage clear, pick applies + persists to next stage, wave-flow QA. Defer fully-randomized (non-drafted) mode if scope balloons — drafted is the priority.
### Phase 6 — classes
