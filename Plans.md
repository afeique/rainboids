...
| ENMY-09 | `enemy/enemy-data.js` + AI | Batch-4 types: Devourer/Phantom/Prism-Mirror/Beacon *(A.E8d)* | ENMY-02/03/04 | 🟡 **PARTIAL** — **PHANTOM 6.180.0** (cloak LIVE, gated `enemy.cloak`, MARK-reveal fix, `spawnPhantom`) + **PRISM_MIRROR 6.183.0** (reflect LIVE: `routeBulletToReflect` bullet pre-pass mirroring boss weak-point routing, front-arc tracks player, reflected-dmg cap 6, beams/rear bypass, gated `enemy.reflects`, `spawnPrismMirror`). **TODO:** Devourer(ENMY-02)/Beacon + add all to WAVE_DATA roster |
| ENMY-10 | `enemy/enemy-data.js` + AI | Batch-5: Leech/Null-Drone/Juggernaut/Thornback/Wraithworm + artillery *(A.E8e)* | ENMY-01/05/06/07 | 🟡 **PARTIAL** — **WRAITHWORM 6.181.0** (blink-burrow + telegraph LIVE, gated `enemy.blink`, CHILL→`_frozenUntil` mirror, `spawnWraithworm`) + **NULL_DRONE 6.182.0** (suppress-aura LIVE: per-frame `applySuppressAura` scan + `cooldownRegenScale` in `updateAbilityCooldowns` + HUD cue, gated `enemy.suppressAura`, `spawnNullDrone`). **TODO:** Leech(ENMY-05)/Juggernaut/Thornback + artillery + add all to WAVE_DATA roster |
