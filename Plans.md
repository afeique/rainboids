...
| CD-02 ◐ | `combat/passive-data.js` | ~~add Bloodshield/Bloodlust/Sanguine/Hemoglutton~~ ✅ (6.199–6.201). **No-downsides removal BATCH 1 ✅ 6.208.0:** dropped HOARDERS_GREED +15%taken / FRENZY +30%taken / TWIN_CAST +30%cost (8 of 11 downsides remain). **STILL TODO (later batches, re-anchor per §6c):** PURIST no-crit→+dmg+pierce; GLASS_CANNON+BERSERKERS merge→"+40%→+90% as HP falls" (frees slot)→add **Apex Predator**; GUNSLINGER (drop no-powers→+primary dmg/firerate); HEAT_SINK (drop lockout→vent AoE); FAILSAFE (→system per-hit cap); EYE_OF_THE_STORM/GRAVITY_WELL/OVERFLOW_CAPACITOR reworks; Overclock/Capacitor-Bank passives; `synergies:[ids]` metadata. **⚠ then enemy-HP re-tune + extremes playtest (§7) — USER.** |
